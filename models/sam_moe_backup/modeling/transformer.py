...
class TwoWayTransformer(nn.Module):
    ...
    def forward(
        ...
        return queries, keys, moe_total_loss
    ...
    def __init__(
        ...
        if self.if_adapter:
            ...
            if moe > 0:
                ...
                #self.MLP_Adapter = Adapter(embedding_dim, skip_connect=False, moe=self.moe)  # MLP-adapter, no skip connection
                #self.Adapter = Adapter(embedding_dim, skip_connect=False, moe=self.moe)  # with skip connection
                ...
                self.Adapter = nn.ModuleList([Adapter(embedding_dim, skip_connect=False) for _ in range(moe)])  # with skip connection

                self.gater = nn.ModuleList([nn.Linear(embedding_dim, self.moe, bias=False) for _ in range(1)])
                #self.gater = nn.ModuleList([nn.Linear(embedding_dim, 10, bias=False) for _ in range(6)])
    ...
    def forward(
        ...
        if self.if_adapter:
            if self.moe <= 0:
                ...
                moe_out, moe_loss = self.moe_forward(queries, self.Adapter, training, labels)
        ...
        if self.if_adapter:
            ...
            if self.moe <= 0:
                ...
                moe_out, moe_loss = self.moe_forward(queries, self.MLP_Adapter, training, labels)
                moe_loss_total += moe_loss
        ...
        if self.if_adapter:
            if self.moe <= 0:
                ...
                moe_out, moe_loss = self.moe_forward(keys, self.MLP_Adapter, training, labels)
                moe_loss_total += moe_loss
                keys = keys + moe_out

        keys = self.norm4(keys)

        return queries, keys, moe_loss_total
