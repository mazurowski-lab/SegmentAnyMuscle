...
class TinyViTBlock(nn.Module):
    ...
    def __init__(self,args, dim, input_resolution, num_heads, window_size=7,
        ...
        if self.args.if_encoder_adapter and (self.depth in self.args.encoder_adapter_depths):
            ...
            if moe > 0:
                ...
                #self.MLP_Adapter = Adapter(dim, skip_connect=False, moe=self.moe)  # MLP-adapter, no skip connection
                #self.Space_Adapter = Adapter(dim, skip_connect=False, moe=self.moe)  # with skip connection

                self.MLP_Adapter = nn.ModuleList([Adapter(dim, skip_connect=False) for _ in range(moe)])  # MLP-adapter, no skip connection
                self.Space_Adapter = nn.ModuleList([Adapter(dim, skip_connect=False) for _ in range(moe)])  # with skip connection

                self.gater = nn.ModuleList([nn.Linear(dim, self.moe, bias=False) for _ in range(1)])
                #self.gater = nn.ModuleList([nn.Linear(dim, 10, bias=False) for _ in range(6)])
            ...
            #self.MLP_Adapter2 = Adapter(dim, skip_connect=False)  # MLP-adapter, no skip connection
            #self.MLP_Adapter3 = Adapter(dim, skip_connect=False)  # MLP-adapter, no skip connection
            #self.MLP_Adapter4 = Adapter(dim, skip_connect=False)  # MLP-adapter, no skip connection

            #self.Space_Adapter2 = Adapter(dim)  # with skip connection
            #self.Space_Adapter3 = Adapter(dim)  # with skip connection
            #self.Space_Adapter4 = Adapter(dim)  # with skip connection
    ...
    def moe_forward(self, x, expert, training, labels=None, noise_epsilon=1e-2, printing=False):
        ...
        if 0:
            if printing:
                print(logits[0,:common_moe])
            logits[:,:common_moe] += logits.max(1).values.unsqueeze(1).repeat(1,common_moe) * 2
            logits[:,:common_moe] += 10
            logits /= 10
            if printing:
                print(logits[0,:common_moe], logits.max(1))
    ...
    def forward(self, x, labels=None, training=False, moe_print=False):
        ...
        if self.args.if_encoder_adapter and (self.depth in self.args.encoder_adapter_depths): 
            ...
            if self.moe <= 0:
                ...
                if self.depth == 0 and moe_print:
                    moe_out, moe_loss = self.moe_forward(x, self.Space_Adapter, training, labels, printing=True)
                else:
                    moe_out, moe_loss = self.moe_forward(x, self.Space_Adapter, training, labels)

                moe_total_loss += moe_loss
                x = x + moe_out # append residual connection
            #else:
            #    tmp = []
            #    for i in range(len(x)):
            #        xi = x[i].unsqueeze(0)
            #        if labels[i] == 0:
            #            out = self.Space_Adapter(xi)
            #        elif labels[i] == 1:
            #            out = self.Space_Adapter2(xi)
            #        elif labels[i] == 2:
            #            out = self.Space_Adapter3(xi)
            #        elif labels[i] == 3:
            #            out = self.Space_Adapter4(xi)
            #        tmp.append(out)
            #    x = torch.cat(tmp)
        ...
        if self.args.if_encoder_adapter and (self.depth in self.args.encoder_adapter_depths):
            ...
            if self.moe <= 0:
                ...
                moe_out, moe_loss = self.moe_forward(x, self.MLP_Adapter, training, labels)
                moe_total_loss += moe_loss
                x = x + self.drop_path(self.mlp(x)) + moe_out
            ...
            #        xi = x[i].unsqueeze(0)
            #        if labels[i] == 0:
            #            out = self.MLP_Adapter(xi)
            #        elif labels[i] == 1:
            #            out = self.MLP_Adapter2(xi)
            #        elif labels[i] == 2:
            #            out = self.MLP_Adapter3(xi)
            #        elif labels[i] == 3:
            #            out = self.MLP_Adapter4(xi)
            #        tmp.append(out)
            #    x = x + self.drop_path(self.mlp(x)) + self.scale * torch.cat(tmp)
        else:
            x = x + self.drop_path(self.mlp(x))
        return x, moe_total_loss
    ...
    def forward(self, x, labels=None, training=False, moe_print=False):
        ...
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                x, curr_loss = blk(x, labels=labels, training=training, moe_print=moe_print)
                moe_loss += curr_loss
        if self.downsample is not None:
            x = self.downsample(x)
        return x, moe_loss
...
class TinyViT(nn.Module):
    ...
    def set_layer_lr_decay(self, layer_lr_decay):
        ...
        #self.patch_embed1.apply(lambda x: _set_lr_scale(x, lr_scales[0]))
        #self.patch_embed2.apply(lambda x: _set_lr_scale(x, lr_scales[0]))
        #self.patch_embed3.apply(lambda x: _set_lr_scale(x, lr_scales[0]))
    ...
    def forward_features(self, x, labels=None, training=False, moe_print=False):
        ...
        #        elif labels[b] == 1:
        #            xi = self.patch_embed1(xi)
        #        elif labels[b] == 2:
        #            xi = self.patch_embed2(xi)
        #        elif labels[b] == 3:
        #            xi = self.patch_embed3(xi)
        #        tmp.append(xi)
        #    x = torch.cat(tmp)

        #    #print(self.patch_embed.seq[0].c.weight[10][1][1])
        #    #print(self.patch_embed1.seq[0].c.weight[10][1][1])
        #    #print(self.patch_embed2.seq[0].c.weight[10][1][1])
        #    #print(self.patch_embed3.seq[0].c.weight[10][1][1])
        ...
        for i in range(start_i, len(self.layers)):
            layer = self.layers[i]
            x, moe_loss = layer(x, labels=labels, training=training, moe_print=moe_print)
    ...
    def forward(self, x, labels=None, training=False, moe_print=False):
        x, moe_loss = self.forward_features(x, labels=labels, training=training, moe_print=moe_print)
        #x = self.norm_head(x)
        #x = self.head(x)
        return x, moe_loss
