    ...
    if checkpoint is not None:
        with open(checkpoint, "rb") as f:
            ...
            for name in state_dict:
                tmp[name] = state_dict[name]
                if 'patch_embed' in name:
                    name_new = name.replace('patch_embed', 'patch_embed1')
                    tmp[name_new] = state_dict[name]
                    name_new = name.replace('patch_embed', 'patch_embed2')
                    tmp[name_new] = state_dict[name]
                    name_new = name.replace('patch_embed', 'patch_embed3')
                    tmp[name_new] = state_dict[name]
            state_dict = tmp
        try:
